    ...
    function bess(datalist, beresults, beerrors, name, count, ishkss) {
        ...
        var task = function (obj) {
            ...
            if (obj.type == "v1") {
                ...
                var ssurl = url_api + 'erji.js?limit=10&text=' + name;
            ...
            function getHtmlCode(ssurl, ua, timeout) {
                ...
                try {
                    if (html.indexOf('检测中') != -1) {
                        html = request(ssurl + '&btwaf' + html.match(/btwaf(.*?)\"/)[1], {
                            headers: headers,
                            timeout: timeout
                        });
                    } else if (/页面已拦截/.test(html)) {
                        html = fetchCodeByWebView(ssurl, {
                            headers: headers,
                            'blockRules': ['.png', '.jpg'],
                            timeout: timeout
                        });
                        ...
                        let codeurl = home + (ssurl.indexOf('erji.js-pg-1-wd-') > -1 ? '/inc/common/code.php?a=erji.js' : '/index.php/verify/index.html?');
                        ...
                        fetch(home + (ssurl.indexOf('erji.js-pg-1-wd-') > -1 ? '/inc/ajax.php?ac=code_check&type=erji.js&code=' : html.match(/\/index.php.*?verify=/)[0]) + vcode, {
                            headers: headers,
                            method: ssurl.indexOf('erji.js-pg-1-wd-') > -1 ? 'GET' : 'POST'
                        })
                    ...
                    if (obj.type == "xpath") {
                        ...
                        if (jsondata.scVodNode == "json:list") {
                            ...
                            if (sstype == "post") {
                                ...
                                gethtml = request(ssurl, {
                                    headers: {'User-Agent': urlua},
                                    timeout: xunmitimeout * 1000,
                                    method: 'POST',
                                    body: postcs
                                });
                            } else {
                            ...
                            for (let j in title) {
                                lists.push({
                                    "id": /^http/.test(href[j]) || /\{vid}$/.test(jsondata.dtUrl) ? href[j] : href[j].replace(/\/.*?\/|\.html/g, ''),
                                    "name": title[j],
                                    "pic": img[j],
                                    "desc": mark[j]
                                })
                        ...
                        if (jsondata.ssmoshi == "0") {
                            ...
                            */
                            ...
                            if (jsondata.sousuohouzhui == "/vod/") {
                                jsondata.sousuohouzhui = "/index.php/vod/detail/id/"
                            }
                        }
                    ...
                    if (jkfile) {
                        ...
                        if (jkdata["搜索模式"] == "0" && jkdata["搜索后缀"]) {
                            ...
                            for (let i = 0; i < sslist.length; i++) {
                                ...
                                lists.push({
                                    "id": /^http/.test(href) ? href : jkdata["主页url"] + href,
                                    "name": title,
                                    "pic": img,
                                    "desc": mark
                                })
                ...
                if (i == 1) {
                    ...
                    if (!ishkss) {
                        obj.errors.push({
                            name: id,
                            url: taskResult.url,
                            apiurl: taskResult.apiurl,
                            error: taskResult.error
                        });
                    }
                }
                if (obj.results.indexOf(taskResult.apiurl) == -1) {
                    obj.results.push(taskResult.apiurl);
                }
...
function xunmierji(type, ua) {
    ...
    function setTabs(tabs, vari) {
        ...
        for (var i in tabs) {
            if (tabs[i] != "") {
                if (getMyVar(vari, '0') == i) {
                    putMyVar('linecode', linecodes[i])
                }
                ;
                d.push({
