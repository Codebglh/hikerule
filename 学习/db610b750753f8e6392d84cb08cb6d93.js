        ...
        if (typeof (typeurl) != "undefined") {
            ...
            if (typeclass && typeclass.length > 0) {
                ...
                for (let i in typeclass) {
                    if (type_pids.indexOf(typeclass[i].type_pid) == -1) {
                        type_pids.push(typeclass[i].type_pid)
                    }
                    if (type_ids.indexOf(typeclass[i].type_id) == -1) {
                        type_ids.push(typeclass[i].type_id)
                    }
                }
...
function sousuo2() {
    ...
    var searchurl = $('').lazyRule(() => {
        ...
        if (getItem('searchmode') == "hiker") {
            return "hiker://erji.js?rule=" + MY_RULE.title + "&s=" + input;
        } else {
